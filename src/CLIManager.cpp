    ...
    try
    {
        ...
        case START:
            ...
            case 3:
            {
                ...
                try
                {
                    students.saveToFile("data/students.txt", [](const Student &s)
                                        { return s.toString(); });
                    teachers.saveToFile("data/teachers.txt", [](const Teacher &t)
                                        { return t.toString(); });
                }
                catch (const std::exception &e)
                {
...
CLIManager::CLIManager()
{
    ...
    try
    {
        students.loadFromFile("data/students.txt", [](const std::string &s)
                              { return Student::fromString(s); });
    }
    catch (...)
    {
    }
    try
    {
        teachers.loadFromFile("data/teachers.txt", [](const std::string &s)
                              { return Teacher::fromString(s); });
    }
    catch (...)
    {
    }
    try
    {
        courses.loadFromFile("data/courses.txt", [](const std::string &c)
                             { return Course::fromString(c); });
    }
    catch (...)
    {
    }
    ...
    try
    {
        GradesManager::loadGrades(students);
    }
    catch (...)
    {
    }
    ...
    while (true)
    {
        ...
        case 5:
        {
            try
            {
                ...
                teachers.saveToFile("data/teachers.txt", [](const Teacher &t)
                                    { return t.toString(); });
                courses.saveToFile("data/courses.txt", [](const Course &c)
                                   { return c.toString(); });
                GradesManager::saveGrades(students);
            }
            catch (const std::exception &e)
            {
                std::cout << "Warning: failed to save data: " << e.what() << "\n";
        ...
        case 6:
        {
            ...
            students.sortBy([](const Student &a, const Student &b)
        ...
        case 10:
        {
            ...
            std::cout << "Course ID (e.g., CS101): ";
            std::cin >> id;
            std::cout << "Course Name: ";
            std::cin >> std::ws;
            std::getline(std::cin, name);
            std::cout << "Teacher Login (assign coordinator): ";
            std::cin >> tLogin;

            Course c(id, name, tLogin);
            courses.add(c);
            std::cout << "Course '" << name << "' created successfully.\n";
            break;
        }
        default:
            break;
...
void CLIManager::teacherMenu()
{
    ...
    auto it = teachers.find_if([this](const Teacher &t)
                               { return t.getLogin() == _currentUser.getLogin(); });
    ...
    while (true)
    {
        ...
        case 2:
        {
            ...
            double value;
            std::cout << "Student login: ";
            std::cin >> studentLogin;
            auto sit = students.find_if([&](const Student &s)
                                        { return s.getLogin() == studentLogin; });
            if (sit == students.getAll().end())
            {
...
void CLIManager::studentMenu()
{
    ...
    auto it = students.find_if([this](const Student &s)
                               { return s.getLogin() == _currentUser.getLogin(); });
    ...
    while (true)
    {
        ...
        case 1:
            ...
            if (me.getEnrolledCourses().empty())
                std::cout << "(none)";
            for (const auto &cId : me.getEnrolledCourses())
            {
                std::cout << cId << " ";
            }
            std::cout << "\n";
        ...
        case 3:
        {
            ...
            me.enroll(courseId);
            std::cout << "Successfully enrolled in " << courseId << ".\n";
            std::cout << "(Remember to select 'Save data' from the admin menu or exit safely to save this change!)\n";
            break;
        }
        default:
            break;
